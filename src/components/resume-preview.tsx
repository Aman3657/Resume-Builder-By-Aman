...
const ResumePreview = React.forwardRef<HTMLDivElement, ResumePreviewProps>(({ resumeData }, ref) => {
  ...
  return (
    <div ref={ref} className="bg-white text-gray-800 p-8 shadow-lg w-full h-full aspect-[210/297] overflow-auto">
      ...
      <section>
        <h2 className="font-headline text-2xl font-bold border-b-2 border-gray-200 pb-2 mb-4">Experience</h2>
        <div className="space-y-6">
          {experience.map(exp => (
            <div key={exp.id}>
              <div className="flex justify-between items-baseline">
                <h3 className="text-lg font-bold">{exp.role || 'Role'}</h3>
                <div className="text-sm text-gray-600">{exp.startDate} - {exp.endDate}</div>
              </div>
              <h4 className="text-md font-semibold text-gray-700">{exp.company || 'Company'}</h4>
      ...
      <section>
        <h2 className="font-headline text-2xl font-bold border-b-2 border-gray-200 pb-2 mb-4">Education</h2>
        <div className="space-y-4">
          {education.map(edu => (
            <div key={edu.id}>
              <div className="flex justify-between items-baseline">
                <h3 className="text-lg font-bold">{edu.institution || 'Institution'}</h3>
                <div className="text-sm text-gray-600">{edu.startDate} - {edu.endDate}</div>
              </div>
              <p className="text-md text-gray-700">{edu.degree || 'Degree or Certificate'}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section>
        <h2 className="font-headline text-2xl font-bold border-b-2 border-gray-200 pb-2 mb-4">Skills</h2>
        <div className="flex flex-wrap gap-2">
          {skills.map(skill => (
            <span key={skill.id} className="bg-accent text-accent-foreground rounded-md px-3 py-1 text-sm font-medium">
              {skill.name || 'Skill'}
            </span>
          ))}
        </div>
